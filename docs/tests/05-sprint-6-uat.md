...
| A4 | Alerta Automática | Ir a la pestaña **Plazos y Términos**. | Debe existir un nuevo plazo: "Revisión de Medida: Prisión Preventiva" con fecha a 2 años. | ⏳ |
...
| C3 | Verificación Adjunto | En el timeline, clic en el archivo adjunto (📎). | Se abre el archivo en nueva pestaña (URL firmada S3). | ⏳ |
| C4 | Filtro por Usuario | Usar dropdown "Todos los Usuarios" > Seleccionar otro usuario (si existe). | El timeline se filtra. Si no tiene actividades, muestra "No hay actuaciones". | ⏳ |
...
- **Alertas:** Las medidas cautelares de tipo "Prisión Preventiva" generan automáticamente un `Deadline` fatal.
- **Estados:**
    - Soluciones: `Propuesta` -> `Aprobada` -> `Cumplida` / `Revocada`.
    - Medidas: `Vigente` -> `Revocada` / `Cumplida`.
- **Seguridad:** Solo el creador de una actividad (o un Admin/Owner en el futuro) puede eliminarla de la bitácora.
