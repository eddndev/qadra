...
#### **2.1.1 TENANT (Workspace / Despacho)**

Representa un despacho legal (tenant del sistema multi-tenant).

**Atributos:**
- `id` (PK)
- `name` (nombre del despacho)
- `slug` (identificador único, ej: bufete-martinez)
- `owner_id` (FK a users - propietario/host del despacho)
- `subscription_tier_id` (FK a subscription_tiers)
- `subscription_status` (active, trial, suspended, cancelled)
- `trial_ends_at` (fecha fin de trial)
- `subscription_ends_at` (fecha fin de suscripción)
- `logo_url` (logo del despacho)
- `address`, `phone`, `email`, `tax_id` (información del despacho)
...
- `hasMany(Case)` → cases
- `belongsTo(SubscriptionTier)` → tier
- `hasMany(Subscription)` → subscriptions (historial de suscripciones vía Cashier)

---

#### **2.1.2 USER (Usuario)**

Representa un usuario del sistema (puede pertenecer a uno o varios despachos).
...
**Relaciones:**
- `belongsToMany(Tenant)` → tenants (via tabla pivot `tenant_user`)
- `hasMany(Tenant, 'owner_id')` → ownedTenants
- `hasMany(Case)` → assignedCases
- `hasMany(CaseActivity)` → activities

**Nota:** La tabla pivot `tenant_user` incluirá campos adicionales:
- `role` (owner, litigante, asociado, paralegal, administrativo, cliente)
- `invited_by` (FK a users)
- `invited_at`, `joined_at`

---

#### **2.1.3 CASE (Caso/Expediente Penal)**

Entidad central. Representa un caso penal gestionado por el despacho.

**Atributos:**
- `id` (PK)
- `tenant_id` (FK a tenants) 🔒 **TENANT SCOPE**
- `case_number` (número de carpeta de investigación, ej: FGR/123/2025)
- `internal_code` (código interno del despacho)
- `crime_type` (tipo de delito, ej: "Homicidio", "Robo", "Fraude")
- `crime_classification` (doloso/culposo)
- `current_stage` (investigation_initial, investigation_complementary, intermediate, trial, execution)
- `current_status` (estado específico según etapa, ver sección 1.6)
- `opened_at` (fecha de inicio del caso)
- `closed_at` (fecha de cierre)
- `assigned_to` (FK a users - abogado responsable principal)
- `representation_type` (defensa, acusación_coadyuvante, asesoría_víctima)
- `court` (juzgado asignado)
- `judge_name` (nombre del juez)
- `prosecutor_name` (nombre del MP)
...
- `belongsTo(User, 'assigned_to')` → responsibleLawyer
- `belongsTo(User, 'created_by')` → creator
- `hasMany(CaseParticipant)` → participants
- `hasMany(CaseDocument)` → documents
- `hasMany(CaseHearing)` → hearings
- `hasMany(CaseActivity)` → activities
- `hasMany(CaseCautelarMeasure)` → cautelarMeasures
- `hasMany(CaseAlternativeSolution)` → alternativeSolutions
- `hasMany(CaseDeadline)` → deadlines

---

#### **2.1.4 CASE_PARTICIPANT (Participante del Caso)**

Representa a las personas involucradas en un caso (imputados, víctimas, testigos, etc.).

**Atributos:**
- `id` (PK)
- `case_id` (FK a cases)
- `tenant_id` (FK a tenants) 🔒 **TENANT SCOPE**
- `type` (imputado, victima, testigo, perito, ministerio_publico, juez, asesor_victima)
- `full_name`
- `identification_number` (CURP, RFC, pasaporte)
- `date_of_birth`
- `gender`
- `phone`, `email`, `address`
- `notes` (observaciones)
...
#### **2.1.5 CASE_DOCUMENT (Documento/Evidencia)**

Almacena archivos relacionados con un caso.

**Atributos:**
- `id` (PK)
- `case_id` (FK a cases)
- `tenant_id` (FK a tenants) 🔒 **TENANT SCOPE**
- `type` (denuncia, querella, ampliacion, solicitud, oficio, acuerdo, evidencia, pericial, acta_audiencia, sentencia, otro)
- `category` (datos_prueba, medios_prueba, prueba_admitida, documento_procesal)
...
- `file_size` (bytes)
- `mime_type`
- `issued_at` (fecha de emisión del documento)
- `issuer` (autoridad que emitió, ej: "Juzgado 5to Penal")
...
**Límites de Storage:**
- Se contabiliza `file_size` por tenant para límites del tier

---

#### **2.1.6 CASE_HEARING (Audiencia)**

Representa audiencias programadas o realizadas.

**Atributos:**
- `id` (PK)
- `case_id` (FK a cases)
- `tenant_id` (FK a tenants) 🔒 **TENANT SCOPE**
- `type` (audiencia_inicial, formulacion_imputacion, vinculacion_proceso, audiencia_intermedia, juicio_oral, otra)
- `title` (ej: "Audiencia de Vinculación a Proceso")
- `scheduled_at` (fecha y hora programada)
- `court` (juzgado)
- `courtroom` (sala)
- `judge_name`
- `attendees` (JSON con lista de asistentes)
- `status` (programada, en_curso, celebrada, suspendida, cancelada)
- `result` (resultado de la audiencia, ej: "Vinculado a proceso por homicidio doloso")
- `notes` (observaciones)
- `created_by` (FK a users)
- `created_at`, `updated_at`, `deleted_at`
...
#### **2.1.7 CASE_ACTIVITY (Actuación/Diligencia)**

Bitácora de actuaciones procesales del caso.

**Atributos:**
- `id` (PK)
- `case_id` (FK a cases)
- `tenant_id` (FK a tenants) 🔒 **TENANT SCOPE**
- `type` (denuncia, querella, citatorio, notificacion, solicitud, comparecencia, oficio, acuerdo, otro)
- `title` (ej: "Solicitud de Orden de Aprehensión")
- `description`
- `performed_at` (fecha de la actuación)
- `responsible_id` (FK a users - responsable)
...
- `belongsTo(Case)` → case
- `belongsTo(Tenant)` → tenant
- `belongsTo(User, 'responsible_id')` → responsible
- `belongsTo(User, 'created_by')` → creator

---

#### **2.1.8 CASE_CAUTELAR_MEASURE (Medida Cautelar)**

Medidas cautelares impuestas al imputado.

**Atributos:**
- `id` (PK)
- `case_id` (FK a cases)
- `tenant_id` (FK a tenants) 🔒 **TENANT SCOPE**
- `type` (presentacion_periodica, embargo, prohibicion_salir_pais, prohibicion_acercamiento, arresto_domiciliario, prision_preventiva, otra)
- `description`
- `imposed_at` (fecha de imposición)
- `expires_at` (fecha de vencimiento, si aplica)
- `status` (activa, cumplida, revocada)
- `notes`
- `created_at`, `updated_at`, `deleted_at`
...
#### **2.1.9 CASE_ALTERNATIVE_SOLUTION (Solución Alterna)**

Soluciones alternas del proceso (acuerdos reparatorios, suspensión condicional, procedimiento abreviado).

**Atributos:**
- `id` (PK)
- `case_id` (FK a cases)
- `tenant_id` (FK a tenants) 🔒 **TENANT SCOPE**
- `type` (acuerdo_reparatorio, suspension_condicional, procedimiento_abreviado)
- `status` (en_negociacion, aprobado, en_cumplimiento, cumplido, incumplido)
- `approved_at` (fecha de aprobación)
- `conditions` (JSON con condiciones a cumplir)
- `completion_deadline` (plazo de cumplimiento)
- `completed_at`
- `notes`
- `created_at`, `updated_at`, `deleted_at`
...
#### **2.1.10 CASE_DEADLINE (Plazo Procesal)**

Control de plazos críticos del proceso.

**Atributos:**
- `id` (PK)
- `case_id` (FK a cases)
- `tenant_id` (FK a tenants) 🔒 **TENANT SCOPE**
- `type` (audiencia_inicial_72h, investigacion_complementaria, presentacion_acusacion, otro)
- `description` (ej: "Cierre de investigación complementaria")
- `deadline_at` (fecha límite)
- `alert_days_before` (días antes para alertar, ej: 7, 3, 1)
...
#### **2.1.11 SUBSCRIPTION_TIER (Tier de Suscripción)**

Define los planes de suscripción disponibles.

**Atributos:**
- `id` (PK)
- `name` (ej: "Starter", "Professional", "Enterprise")
- `slug` (starter, professional, enterprise)
- `description`
- `price_monthly` (precio mensual en centavos)
- `price_yearly` (precio anual en centavos)
- `stripe_product_id`
- `stripe_price_monthly_id`
- `stripe_price_yearly_id`
- `max_active_cases` (límite de casos activos, null = ilimitado)
- `max_users` (límite de miembros del equipo, null = ilimitado)
- `max_storage_gb` (límite de almacenamiento en GB)
- `features` (JSON con funcionalidades habilitadas)
- `is_active` (boolean)
- `sort_order`
- `created_at`, `updated_at`

**Relaciones:**
- `hasMany(Tenant)` → tenants

**Features por Tier (ejemplo):**
```json
{
  "client_portal": false,
...
### 2.2 Diagrama Entidad-Relación (Mermaid)

```mermaid
erDiagram
    TENANT ||--o{ USER : "has many members"
    TENANT ||--o{ CASE : "manages"
    TENANT ||--o| SUBSCRIPTION_TIER : "belongs to"
    USER ||--o{ CASE : "assigned to"
    USER ||--o{ CASE_ACTIVITY : "creates"
    CASE ||--o{ CASE_PARTICIPANT : "has"
    CASE ||--o{ CASE_DOCUMENT : "has"
    CASE ||--o{ CASE_HEARING : "has"
    CASE ||--o{ CASE_ACTIVITY : "has"
    CASE ||--o{ CASE_CAUTELAR_MEASURE : "has"
    CASE ||--o{ CASE_ALTERNATIVE_SOLUTION : "has"
    CASE ||--o{ CASE_DEADLINE : "has"
    ...
    TENANT {
        ...
        string slug UK
        bigint owner_id FK
        bigint subscription_tier_id FK
    ...
    CASE_CAUTELAR_MEASURE {
        bigint id PK
        bigint case_id FK
        bigint tenant_id FK
        string type
        timestamp imposed_at
        timestamp expires_at
        string status
        timestamps
    }

    CASE_ALTERNATIVE_SOLUTION {
        bigint id PK
        bigint case_id FK
        bigint tenant_id FK
        string type
