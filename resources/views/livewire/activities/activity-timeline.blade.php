<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1">
        <div class="bg-white shadow rounded-lg p-6 sticky top-6">
            <h3 class="text-lg font-medium text-gray-900 mb-4">Registrar Actuación</h3>
            ...
            <form wire:submit.prevent="save">
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Título Breve *</label>
                        <input type="text" wire:model="title" placeholder="Ej: Llamada con MP Lic. Torres"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" wire:model="performed_at"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    ...
                    <div>
                        ...
                        <input type="number" wire:model="duration_minutes" placeholder="30"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Adjuntar Archivos (Opcional)</label>
                        <input type="file" wire:model="attachments" multiple class="mt-1 block w-full text-sm text-gray-500
                        ...
                        " />
                        <div wire:loading wire:target="attachments" class="text-xs text-indigo-600 mt-1">Subiendo...
                        </div>
                    </div>
                                ...
                                <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                    <div>
                                        ...
                                        @if($activity->description)
                                            <p
                                                class="mt-2 text-sm text-gray-700 whitespace-pre-line bg-gray-50 p-3 rounded-lg border border-gray-200">
                                        ...
                                        @if($activity->media->count() > 0)
                                            <div class="mt-2 flex flex-wrap gap-2">
                                                @foreach($activity->media as $media)
                                                    <a href="{{ $media->getTemporaryUrl(now()->addMinutes(5)) }}" target="_blank"
                                    ...
                                    <div class="text-right text-sm whitespace-nowrap text-gray-500">
                                        <time
                                            datetime="{{ $activity->performed_at }}">{{ $activity->performed_at->diffForHumans() }}</time>
                                        ...
                                        @if($activity->performed_by === auth()->id())
                                            <button wire:click="delete('{{ $activity->id }}')"
                                                wire:confirm="¿Eliminar esta actuación?"
                                                class="block mt-2 text-xs text-red-500 hover:text-red-700 ml-auto">
                                                Eliminar
                                            </button>
                                        @endif
                                    </div>
