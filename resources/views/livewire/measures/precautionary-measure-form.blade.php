<div>
    ...
    @if(!$showForm)
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-medium text-gray-900">Medidas Cautelares Vigentes</h3>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="mt-2 sm:flex sm:justify-between">
                                    <div class="sm:flex">
                                        <p class="flex items-center text-xs text-gray-500 mr-6">
                            ...
                            <div class="ml-4 flex-shrink-0 flex flex-col gap-2">
                                <button wire:click="edit('{{ $measure->id }}')"
                                    class="text-indigo-600 hover:text-indigo-900 text-sm">Editar</button>
            ...
            <form wire:submit.prevent="save">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="col-span-1 md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700">Imputado *</label>
                        <select wire:model="participant_id"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="">-- Seleccionar --</option>
                            @foreach($imputed_participants as $imputed)
                                <option value="{{ $imputed->id }}">{{ $imputed->name }}
                                    {{ $imputed->is_detained ? '(Detenido)' : '' }}</option>
                            @endforeach
                        </select>
                        @error('participant_id') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        @if($imputed_participants->isEmpty())
                            <span class="text-xs text-orange-500 block mt-1">⚠ No hay imputados registrados en este caso.
                                Agregue participantes primero.</span>
                        @endif
                    </div>

                    <!-- Tipo de Medida -->
                    <div class="col-span-1 md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700">Tipo de Medida *</label>
                        <select wire:model.live="measure_type_id"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <!-- Fechas -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Fecha de Imposición *</label>
                        <input type="date" wire:model="imposed_at"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @error('imposed_at') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Fecha de Revisión Obligatoria</label>
                        <input type="date" wire:model="review_date"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <!-- Descripción -->
                    <div class="col-span-1 md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700">Descripción Detallada / Condiciones *</label>
                        <textarea wire:model="description" rows="3"
                            placeholder="Especifique montos, direcciones, horarios o restricciones..."
                    ...
                    <!-- Juez -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Juez que impuso</label>
