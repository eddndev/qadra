<header class="bg-white/90 backdrop-blur-sm shadow-sm h-16 flex justify-between items-center px-6 sticky top-0 z-40">
    ...
    <div class="flex items-center lg:hidden mr-4">
        <button type="button" onclick="document.getElementById('mobile-sidebar').classList.toggle('hidden')"
            ...
            <x-dropdown align="right" width="48">
                <x-slot name="trigger">
                    <button class="flex items-center gap-2 focus:outline-none">
                        @php $freshUser = Auth::user()->fresh(['media']);
                        $avatarUrl = $freshUser?->getAvatarUrl(); @endphp
                        @if($avatarUrl)
                            <img class="h-8 w-8 rounded-full object-cover border border-gray-200" src="{{ $avatarUrl }}"
                                alt="{{ $freshUser->name }}" />
                        @else
                            <div
                                class="h-8 w-8 rounded-full bg-slate-700 text-white flex items-center justify-center text-sm font-semibold">
