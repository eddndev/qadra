<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="GET" action="{{ route('reports.index') }}"
                class="flex flex-wrap gap-4 bg-white p-3 rounded-lg border border-gray-200 shadow-sm items-center">
                <select name="time_range" onchange="this.form.submit()"
                    class="block w-40 pl-3 pr-10 py-1.5 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                    <option value="30" {{ $filters['time_range'] == '30' ? 'selected' : '' }}>Últimos 30 días</option>
                    <option value="this_year" {{ $filters['time_range'] == 'this_year' ? 'selected' : '' }}>Este año
                    </option>
                    <option value="last_year" {{ $filters['time_range'] == 'last_year' ? 'selected' : '' }}>Año pasado
                    </option>
                </select>
                <select name="stage" onchange="this.form.submit()"
                    class="block w-40 pl-3 pr-10 py-1.5 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                    <option value="">Todas las etapas</option>
                    <option value="Investigación" {{ $filters['stage'] == 'Investigación' ? 'selected' : '' }}>
                        Investigación</option>
                    <option value="Juicio Oral" {{ $filters['stage'] == 'Juicio Oral' ? 'selected' : '' }}>Juicio Oral
                    </option>
                </select>
                <input type="text" name="court" value="{{ $filters['court'] }}" placeholder="Buscar juzgado..."
                    class="block w-40 pl-3 pr-3 py-1.5 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
            ...
            <div class="mt-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="bg-white rounded-lg p-4 border border-gray-200 shadow-sm relative">
                        <div
                            class="absolute top-4 right-4 {{ $totalGrowth >= 0 ? 'text-green-500 bg-green-50' : 'text-red-500 bg-red-50' }} text-xs font-bold px-1.5 py-0.5 rounded">
                    ...
                    <div class="bg-white rounded-lg p-4 border border-gray-200 shadow-sm relative">
                        <div
                            class="absolute top-4 right-4 {{ $closedGrowth >= 0 ? 'text-green-500 bg-green-50' : 'text-red-500 bg-red-50' }} text-xs font-bold px-1.5 py-0.5 rounded">
                            {{ $closedGrowth >= 0 ? '+' : '' }}{{ $closedGrowth }}%
                        </div>
                        <div class="flex items-center gap-3 mb-2">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
                ...
                <div class="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                    ...
                    <div class="mt-4 flex justify-center gap-3 text-xs">
                        <div class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-blue-600"></span>
                            Acuerdo</div>
                        <div class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-green-500"></span>
                            Ganado</div>
                        <div class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-slate-400"></span>
                            Pendiente</div>
                    </div>
                </div>
            </div>
                            ...
                            @foreach($cases as $case)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-xs text-gray-500">
                                        {{ ucfirst($case->stage ?? 'No definida') }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-xs text-gray-500">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                            $hasAlert = $case->deadlines->where('expires_at', '<', now())->where('status', '!=', 'completed')->isNotEmpty();
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-xs text-gray-500">
                                        <span
                                            class="bg-slate-50 text-slate-700 px-2 py-0.5 rounded border border-slate-100">{{ ucfirst($case->status) }}</span>
                                    </td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const resultsData = @json($resultsDistribution);
