                            ...
                            @forelse($expiredDeadlines as $deadline)
                                <div class="p-4 hover:bg-red-100/30 transition-colors">
                                    ...
                                    <div class="text-sm font-medium text-gray-800 mb-1">
                                        {{ $deadline->case->crime_type ?? 'Delito' }} -
                                        {{ $deadline->case->case_alias ?? 'Alias' }}
                                    </div>
                                    ...
                                    <div class="flex items-center justify-between text-xs text-red-700">
                                        ...
                                        <span class="font-bold">Vencido hace {{ $deadline->expires_at->diffInDays(now()) }}
                                            días</span>
                                    </div>
                                    <div class="mt-2">
                                        <a href="{{ route('cases.show', $deadline->case_id) }}"
                                ...
                                <div class="p-4 hover:bg-blue-100/30 transition-colors">
                                    <div class="flex justify-between items-start mb-1">
                                        ...
                                        <span
                                            ...
                                            {{ $deadline->expires_at->diffForHumans() }}
                                        </span>
                                    </div>
                                    <div class="text-sm font-medium text-gray-800 mb-1">
                                        {{ $deadline->case->crime_type ?? 'Delito' }} -
                ...
                <div class="space-y-8">
                    ...
                    <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <span
                                class="bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded-full font-bold">{{ count($recentActivities) }}</span>
                        </div>
                        ...
                        @else
                            <div class="relative pl-4 border-l-2 border-gray-100 space-y-6">
                                @foreach($recentActivities as $activity)
                                    ...
                                    <div class="relative">
                                        <div
                                            class="absolute -left-[21px] {{ $colorClass }} p-1 rounded-full border border-white">
                                            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <div class="text-xs text-gray-400 mt-1">
                                            ...
                                            @if($activity->legalCase)
                                                • <a href="{{ route('cases.show', $activity->legalCase->id) }}"
