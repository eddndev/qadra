<section>
    ...
    <div class="flex items-start gap-6 mb-8 pb-8 border-b border-gray-100" x-data="{ avatar: null, avatarPreview: null }">
        <div class="relative group cursor-pointer" @click="document.getElementById('avatar-input').click()">
            <!-- Current Avatar / Initials -->
            <div class="h-24 w-24 rounded-full bg-gray-200 flex items-center justify-center text-3xl font-bold text-gray-500 overflow-hidden relative">
                <!-- If user has avatar, show it -->
                @if($user->hasMedia('avatar'))
                    <img src="{{ $user->getAvatarUrl() }}" alt="{{ $user->name }}" class="h-full w-full object-cover" x-show="!avatarPreview">
                @else
            ...
            <button type="button" class="absolute bottom-0 right-0 bg-[#1E293B] text-white p-2 rounded-full hover:bg-gray-700 transition-colors z-10">
    ...
    <form method="post" action="{{ route('profile.update') }}" class="space-y-6" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <x-text-input id="email" name="email" type="email" class="mt-1 block w-full bg-slate-100 cursor-not-allowed" :value="old('email', $user->email)" required readonly autocomplete="username" />
        ...
        <div class="mt-6">
            ...
            <div
                class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer group">
